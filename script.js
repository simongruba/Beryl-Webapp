...
mergeClassRoster(state, window.BERYL_ROSTER || []);
...
const BERYL_MEDIA_BUCKET = "beryl-media";
...
async function restoreBerylSession() {
  if (cloudBusy || cloudLoading || authBusy || $("#modal").open) return;
  const version = sessionVersion;
  try {
    const restored = await BerylAuth.getCurrentUser();
    ...
    console.error("Beryl loading:", error.message);
    toast(error.message);
  }
}
...
async function flushFileCleanup() {
  ...
  const { data: jobs, error } = await supabaseClient
    .from("beryl_file_cleanup").select("path").limit(1000);
  if (error) throw new Error(error.message);
  for (const job of jobs || []) {
    const { data: inUse, error: usageError } = await supabaseClient
      .rpc("beryl_photo_in_use", { photo_path: job.path });
    if (usageError) throw new Error(usageError.message);
    if (!inUse) {
      const { error: removalError } = await supabaseClient.storage
        .from(BERYL_MEDIA_BUCKET).remove([job.path]);
      ...
      const { data: remaining, error: listError } = await supabaseClient.storage
        .from(BERYL_MEDIA_BUCKET)
        .list(job.path.slice(0, slash), { search: job.path.slice(slash + 1), limit: 100 });
      if (listError) throw new Error(listError.message);
      if ((remaining || []).some(file => file.name === job.path.slice(slash + 1))) {
        throw new Error("A photo could not be deleted. Check the Storage delete policy.");
      }
      photoUrls.delete(job.path);
    }
    const { error: queueError } = await supabaseClient.from("beryl_file_cleanup")
      .delete().eq("path", job.path);
...
async function migrateBrowserPhotos(value) {
  ...
  if (!value.storagePath && typeof value.src === "string" && value.src.startsWith("data:image/")) {
    const blob = await (await fetch(value.src)).blob();
    const uploaded = await readImage(new File([blob], "imported-photo", { type: blob.type }));
    delete value.src;
...
async function loadClassroom() {
  ...
  cloudLoading = true;
  try {
    let { data, error } = await supabaseClient.from("beryl_classroom_content")
      .select("content,revision").eq("id", 1).maybeSingle();
    if (error) throw new Error(`Classroom could not load. Run 1-setup-database.sql first. ${error.message}`);
    if (generation !== cloudLoadVersion) return;
    if (!data && admin()) {
      // Keep a local recovery copy before the first migration.
      localStorage.setItem(KEY + "-before-cloud", JSON.stringify(legacyClassroom));
      ...
      const inserted = await supabaseClient.from("beryl_classroom_content")
        .insert({ id: 1, content: seed }).select("content,revision").single();
      if (inserted.error) {
        // Another admin may have initialized it while this tab was uploading.
        const existing = await supabaseClient.from("beryl_classroom_content")
          .select("content,revision").eq("id", 1).maybeSingle();
        if (existing.error || !existing.data) throw new Error(inserted.error.message);
        data = existing.data;
      } else data = inserted.data;
    }
    if (!data) throw new Error("Your adviser must sign in once to initialize the classroom.");
    if (generation !== cloudLoadVersion) return;
    applyCloudRow(data);
    try { await flushFileCleanup(); }
    catch (error) { toast(`Photo cleanup needs a retry: ${error.message}`); }
  } finally {
...
async function commit(change) {
  ...
  try {
    change();
    const { data, error } = await supabaseClient.from("beryl_classroom_content")
      .update({ content: classroomContent(state) })
      .eq("id", 1).eq("revision", cloudRevision)
      .select("content,revision").maybeSingle();
    if (error) throw new Error(error.message);
    if (!data) {
      state = before;
      await loadClassroom();
      throw new Error("The classroom changed in another tab. The latest version is loaded; please repeat your edit.");
    }
    saved = true;
    applyCloudRow(data);
    ...
    if (!saved && cloudRevision === null) state = before;
      ...
      state = before;
      try { await loadClassroom(); } catch { cloudReady = false; }
    }
    render();
    modal(saved ? "Saved — photo cleanup needs a retry" : "Change not confirmed", `
...
async function postClassMessage(note) {
  if (!cloudReady || cloudBusy) throw new Error("Please wait for the classroom to load or finish saving.");
  if (!user || !["student", "admin"].includes(user.role)) throw new Error("Your account must be approved to post messages.");
  cloudBusy = true;
  try {
    const { data, error } = await supabaseClient.rpc("beryl_post_message", {
      request_id: note.id,
      note_text: note.text,
...
// A small browser-like history for list popups. Example:
// View all students → student profile → Close → View all students.
// Only popups that explicitly provide a restore callback become history
// parents, so normal editor/auth flows are not affected.
const modalHistory = [];
...
function modal(title, html, options = {}) {
  const dialog = $("#modal");
  // Keep the exact page position where the first popup was opened.
  if (!dialog.open) modalReturnScrollY = window.scrollY;

  // If the currently visible popup is a restorable parent (for example
  // "View all students"), remember how to rebuild it before showing a child.
...
$("#modal").addEventListener("cancel", (e) => {
  ...
  // Escape acts like the × button when a popup was opened from another popup.
  if (modalHistory.length) {
    e.preventDefault();
    close();
  }
...
function login(mode = "signin", prefill = "") {
  ...
  modal(
    creating ? "Join the Beryl classroom" : "Welcome back to Beryl",
    /* HTML */ `
      <div class="auth-experience">
        <div class="auth-welcome-strip">
          <span class="auth-gem" aria-hidden="true">
            <img src="assets/beryl-mark.svg" alt="" width="42" height="42" />
          </span>
          <span class="auth-welcome-copy">
            <small>BERYL CLASSROOM</small>
            <strong>${creating ? "Create your little corner in Beryl" : "Welcome back to our classroom"}</strong>
            <span>${creating ? "Join the class space for notes and memories." : "Sign in to continue where you left off."}</span>
          </span>
        </div>
      <div class="auth-tabs">
        <button
      ...
      <p class="hint">
        ${creating ? "Create your Berylite account to leave messages for the class." : "Use the email and password for your existing account."}
      </p>
      <form id="login-form">
        <fieldset class="auth-fields">
          ...
          ${creating ? '<label class="field">Confirm password<input name="confirmation" type="password" required minlength="8" maxlength="128" autocomplete="new-password"></label><p class="hint">Use at least 8 characters. Your account is saved in Supabase. Confirm your email if requested.</p>' : ""}
  ...
  f.onsubmit = async (e) => {
    ...
    try {
      const result = creating
        ? await BerylAuth.register(input)
        : await BerylAuth.signIn(input);
        ...
        modal(
          ...
          /* HTML */ `
            <div class="session-state">
              ...
              <h3>Welcome to Beryl!</h3>
              <p>
                Your username is
                <strong>${esc(result.username)}</strong>
                . Check your email for a confirmation link if one was sent, then sign in using your email address and password.
              </p>
...
function account() {
  ...
  if (!user) {
    $("#account").innerHTML = /* HTML */ `
      <button id="login" class="pill account-signin">Sign in <span aria-hidden="true">↗</span></button>
    `;
    $("#login").onclick = login;
    return;
  }

  const displayName = String(user.name || user.username || "Berylite");
  const initials = displayName
    ...
    .join("") || "B";
  const roleLabel = admin() ? "Adviser / Admin" : "Student";

  $("#account").innerHTML = /* HTML */ `
    <button id="user-menu" class="pill account-trigger" aria-expanded="false" aria-controls="account-panel">
      <span class="account-trigger-avatar" aria-hidden="true">${esc(initials)}</span>
      <span class="account-trigger-name">${esc(displayName)}</span>
    ...
    <div id="account-panel" class="account-panel" hidden>
      <div class="account-panel-top">
        ...
        <span class="account-identity">
          <small>BERYL ACCOUNT</small>
          <strong>${esc(displayName)}</strong>
          <span>${esc(user.email || "Signed-in account")}</span>
        </span>
      </div>
      <div class="account-meta-grid">
        ...
        <span class="account-meta-item">
          <small>Status</small>
          <strong class="account-online"><i aria-hidden="true"></i> Signed in</strong>
        </span>
      </div>
      <div class="account-panel-note">
        <span aria-hidden="true">✦</span>
        <span>${admin() ? "Classroom tools are unlocked for this session." : "Student account · You can leave messages and enjoy the Beryl classroom."}</span>
      </div>
      <button id="logout" class="account-logout" type="button">
  ...
  const setAccountOpen = (open) => {
    ...
    $("#account").classList.toggle("account-open", open);
  };
  trigger.onclick = (event) => {
    event.stopPropagation();
    setAccountOpen(panel.hidden);
  };
...
document.addEventListener("click", (event) => {
  ...
  const panel = $("#account-panel");
  const trigger = $("#user-menu");
  if (!accountRoot || !panel || panel.hidden || accountRoot.contains(event.target)) return;
  panel.hidden = true;
  accountRoot.classList.remove("account-open");
...
async function readImage(file) {
  ...
  const current = await BerylAuth.getCurrentUser();
  ...
  const bucket = supabaseClient.storage.from(BERYL_MEDIA_BUCKET);
...
function confirmDelete(action) {
  modal(
    "Delete this item?",
    /* HTML */ `
      <p>This removes it from the shared classroom and deletes any photo files no longer used.</p>
      <div class="actions">
...
function showHeroSide(side) {
  const back = Number(side) === 1;
  $("#class-photo").classList.toggle("flipped", back);
  $("#class-photo").setAttribute("aria-label", back
    ? "Showing back class photo. Click to show front."
    : "Showing front class photo. Click to show back.");
  $$("[data-hero-side]").forEach(button => {
    button.setAttribute("aria-pressed", String(Number(button.dataset.heroSide) === Number(back)));
  });
}

function renderHero() {
  $("#class-front").innerHTML = media(state.hero[0], "Front class photo");
  $("#class-back").innerHTML = media(state.hero[1], "Back class photo");
  $("#hero-tools").innerHTML = admin()
    ? [0, 1].map(side => `
        <div class="hero-tool-group">
          <button type="button" class="hero-side-button" data-hero-side="${side}">
            ${side === 0 ? "Front photo" : "Back photo"} <span>View this side ↗</span>
          </button>
          ${mediaTools(`hero:${side}`, state.hero[side])}
        </div>
      `).join("")
    : "";
  showHeroSide($("#class-photo").classList.contains("flipped") ? 1 : 0);
}
$("#class-photo").onclick = () => showHeroSide(
  $("#class-photo").classList.contains("flipped") ? 0 : 1
);
...
function previewStudents(list) {
  for (const student of list) {
    if (!studentPreviewOrder.has(student.id)) studentPreviewOrder.set(student.id, Math.random());
  }
  return [...list]
    .sort((a, b) => studentPreviewOrder.get(a.id) - studentPreviewOrder.get(b.id))
    .slice(0, 5)
...
function studentCard(s) {
  ...
  return /* HTML */ `
    ...
    >
      ...
      <h4>${esc(s.name)}</h4>
      <p class="student-quote">${esc(s.quote || "A story waiting to be told.")}</p>
    </button>
  `;
}
...
function renderStudents() {
  ...
  $("#student-results").textContent = studentQuery.trim()
    ...
    : `${state.students.length} Berylites`;
...
${esc(s.quote)}</textarea>
  ...
  if (admin())
    $("#quote-form").onsubmit = async (e) => {
      ...
      if (await commit(() => (state.students.find((s) => s.id === id).quote = value)))
        toast("Quote saved.");
    };
}
  ...
  return /* HTML */ `
    <article class="officer-card officer-card-adviser">
      <button
        ...
        aria-label="View ${esc(adviser.name || "Beryl Adviser")}, Class Adviser"
      >
        <div class="officer-photo">${media(adviser.image, adviser.name || "Beryl Adviser")}</div>
        <div class="officer-copy">
...
function officerAdviserDetail() {
  ...
  modal(
    ...
    /* HTML */ `
      <div class="mini-profile officer-profile-detail">
        <div class="mini-profile-photo">${media(adviser.image, adviser.name || "Beryl Adviser")}</div>
        <h3>${esc(adviser.name || "Our Adviser")}</h3>
        <p>Class Adviser</p>
      </div>
    `,
...
function officerForm(officer = null) {
  ...
  modal(
    ...
    /* HTML */ `
      <form id="officer-form">
        ...
        <p class="hint">The officer card automatically uses the Berylite’s existing profile photo.</p>
  ...
  $("#officer-form").onsubmit = async (event) => {
    ...
    if (!studentId || !position) return toast("Choose a Berylite and enter a position.");
...
function deleteOfficer(id) {
  ...
  confirmRemoval(
    "Remove this officer?",
    `Remove ${student?.name || "this student"} from the class officers panel? Their Berylite profile will stay in the class directory.`,
    async () => {
...
function renderBirthdays() {
  ...
  if (intro) {
    intro.textContent = todays.length
      ? `Today is extra special — Beryl is celebrating ${todays.length === 1 ? todays[0].student.name : `${todays.length} Berylites`}! Tap the birthday spotlight to celebrate with them.`
      : entries.length
        ? `${monthName} has ${entries.length === 1 ? "one Beryl birthday" : `${entries.length} Beryl birthdays`} worth remembering — tap a card for their spotlight.`
              ...
              <article class="birthday-card ${isToday ? "birthday-today birthday-feature" : ""}">
                ...
                >
                  ...
                  <div class="birthday-card-copy">
                    ...
                    <h3>${esc(student.name)}</h3>
                    <p>${isToday ? "Beryl is celebrating you today ♡" : `${monthName} birthday`}</p>
                  </div>
                </button>
                ${
                  admin()
...
function renderOpeningBirthday() {
  ...
  holder.innerHTML = /* HTML */ `
    <div class="opening-birthday-card">
      <p class="opening-birthday-kicker">TODAY, BERYL CELEBRATES</p>
...
function birthdayForm(birthday = null) {
  ...
  modal(
    birthday ? "Edit birthday" : "Add a Beryl birthday",
    /* HTML */ `
  ...
  $("#birthday-form").onsubmit = async (event) => {
    ...
    if (duplicate) return toast("That Berylite already has a birthday saved. Edit the existing one instead.");
    const ok = await commit(() => {
      state.birthdays ??= [];
      if (birthday) {
...
function birthdayCalendarHTML() {
  ...
  const allEntries = birthdayEntries();
  ...
  const summary = /* HTML */ `
    <div class="birthday-calendar-summary">
      <div class="birthday-calendar-summary-icon" aria-hidden="true">✦</div>
      <strong>${allEntries.length} Beryl birthday${allEntries.length === 1 ? "" : "s"}</strong>
    </div>
  `;
  const manageHint = admin()
    ? `<div class="birthday-calendar-manage-hint"><span aria-hidden="true">✎</span><strong>Admin mode</strong></div>`
    : "";

  return summary + manageHint + `<div class="birthday-calendar-months">` + [...groups.entries()]
    .sort((a, b) => a[0] - b[0])
    .map(
      ([month, entries]) => /* HTML */ `
        <section class="birthday-month-group">
          <h3><span>${esc(MONTHS[month - 1])}</span></h3>
              ...
              .map(
                ({ birthday, student }) => /* HTML */ `
                  <div class="birthday-calendar-item">
                    <button data-birthday-detail="${esc(birthday.id)}" class="birthday-calendar-row" type="button">
                      <strong>${String(birthday.day).padStart(2, "0")}</strong>
                      <span class="birthday-calendar-name">${esc(student.name)}</span>
                      <em aria-hidden="true">↗</em>
                    </button>
                    ${
                      admin()
                        ? `<div class="birthday-calendar-actions">
                             <button class="outline" data-edit-birthday="${esc(birthday.id)}" type="button"><span aria-hidden="true">✎</span> Edit</button>
                             <button class="outline danger-lite" data-delete-birthday="${esc(birthday.id)}" type="button"><span aria-hidden="true">×</span> Remove</button>
                           </div>`
                        : ""
                    }
                  </div>
...
function deleteBirthday(id) {
  ...
  confirmRemoval(
    ...
    `Remove ${student?.name || "this student"} from the birthday calendar? Their Berylite profile will stay in the class directory.`,
    async () => {
...
function note(m, full = false) {
  const color = /^#[0-9a-f]{6}$/i.test(m.color) ? m.color : "#e4efbb";
  const rgb = color
    .slice(1)
    .match(/../g)
    .map((v) => parseInt(v, 16));
...
function messageDetail(id) {
  const m = state.messages.find((m) => m.id === id);
  if (m) modal("A note for Beryl", note(m, true));
}
...
$("#message-form").onsubmit = async (e) => {
  ...
  try {
    ...
    const ok = await postClassMessage({
      id: e.target.dataset.requestId || (e.target.dataset.requestId = crypto.randomUUID()),
      text,
...
function showBirthdayCalendarPopup() {
  modal(
    "Beryl birthday calendar",
    /* HTML */ `<div class="birthday-calendar">${birthdayCalendarHTML()}</div>`,
    { restore: showBirthdayCalendarPopup, className: "birthday-calendar-modal" },
  );
}
...
document.addEventListener("click", (e) => {
  ...
  if (!b) return;
  const d = b.dataset;
  if (d.heroSide !== undefined) {
    showHeroSide(d.heroSide);
  } else if (d.noteColor) {
  ...
  else if (d.editOfficer) officerForm((state.officers || []).find((item) => item.id === d.editOfficer));
  else if (d.deleteOfficer) deleteOfficer(d.deleteOfficer);
  else if (d.editBirthday) birthdayForm((state.birthdays || []).find((item) => item.id === d.editBirthday));
...
// Mobile classroom menu: all Beryl panels live in one burger drawer on phones.
...
function setMobileMenu(open) {
  ...
  if (mobileMenuToggle) {
    mobileMenuToggle.setAttribute(
      "aria-label",
      next ? "Close classroom menu" : "Open classroom menu",
    );
  }
...
window.addEventListener("resize", () => {
  renderMessages();
  renderAlbums();
  if (!matchMedia("(max-width: 760px)").matches) setMobileMenu(false);
});
if (document.modelContext?.registerTool) {
  try {
    Promise.resolve(
      document.modelContext.registerTool({
        name: "show_beryl_encouragement",
        description:
    ...
    /* HTML */ `
      <div class="adviser-profile">
        ...
        <div class="adviser-copy">
          <p class="eyebrow">GRADE 10 · BERYL</p>
          <h3>${esc(a.name || "Your Beryl adviser")}</h3>
...
async function changeSession(nextUser) {
  ...
  const displayName = signingIn ? String(nextUser.name || nextUser.username || "Berylite") : "";
  const initials = displayName
    .split(/[\s._-]+/)
    .filter(Boolean)
    .slice(0, 2)
  ...
  modal(
    signingIn ? "Opening your Beryl account" : "Signing out of Beryl",
    /* HTML */ `
      <div class="session-state session-working" role="status">
        <div class="session-scene" aria-hidden="true">
          <span class="session-orbit session-orbit-one"></span>
          <span class="session-orbit session-orbit-two"></span>
          <span class="session-spark session-spark-one">✦</span>
          <span class="session-spark session-spark-two">✦</span>
          <span class="session-gem"><img src="assets/beryl-mark.svg" alt="" width="58" height="58" /></span>
        </div>
        <small>BERYL CLASSROOM</small>
        <h3>${signingIn ? `Getting things ready for ${esc(displayName)}…` : "Putting everything safely back in place…"}</h3>
        <p>${signingIn ? "Opening your classroom space and syncing the latest memories." : "Saving your visit and closing your account session."}</p>
        <div class="session-progress" aria-hidden="true"><span></span></div>
      </div>
    `,
  ...
  try {
    if (!signingIn) await BerylAuth.signOut();
    user = nextUser;
    state = emptyClassroom();
    cloudReady = false;
    photoUrls.clear();
  ...
  render();
  modal(
    signingIn ? "Welcome to Beryl" : "You’re signed out",
    /* HTML */ `
      <div class="session-state session-complete">
        <div class="session-success" aria-hidden="true">
          <span class="session-check">✓</span>
          <span class="session-success-ring"></span>
        </div>
        <small>BERYL CLASSROOM</small>
        <h3>${signingIn ? `Welcome back, ${esc(displayName)}!` : "See you in the classroom!"}</h3>
        <p>${signingIn ? "Your account is ready and the latest classroom content is loaded." : "Your session has been closed safely. The classroom will be here when you return."}</p>
        ${signingIn ? `
          <div class="session-user-card">
            <span class="session-user-avatar">${esc(initials)}</span>
            <span><strong>${esc(displayName)}</strong><small>${admin() ? "Adviser / Admin" : "Beryl account"}</small></span>
            <i aria-hidden="true"></i>
          </div>
        ` : ""}
        <button class="pill session-done" id="session-done">${signingIn ? "Enter the classroom" : "Back to Beryl"} <span aria-hidden="true">→</span></button>
      </div>
    `,
    { className: "session-modal" },
  );
...
function deleteStudent(id) {
  ...
  confirmRemoval(
    "Delete this student?",
    `Remove ${student.name}, their quote and both profile photos from the shared classroom? This does not delete their sign-in account.`,
    async () => {
      if (
        await commit(() => {
          ...
          state.officers = (state.officers || []).filter((o) => o.studentId !== id);
          state.birthdays = (state.birthdays || []).filter((b) => b.studentId !== id);
        })
      ) {
        close();
...
function deleteAlbum(id) {
  ...
  confirmRemoval(
    ...
    async () => {
      if (
        await commit(() => (state.albums = state.albums.filter((a) => a.id !== id)))
      ) {
        close();
        toast("Album deleted.");
      }
  ...
  modal(
    ...
    /* HTML */ `
      <div class="album-photo-view">
        <img src="${esc(imageSource(photo))}" alt="${esc(album.name)} — full photo" />
      </div>
...
void restoreBerylSession();
setInterval(() => {
  if (!authBusy) void restoreBerylSession();
}, 30 * 1000);
document.addEventListener("visibilitychange", () => {
  if (!document.hidden && !authBusy) void restoreBerylSession();
});
